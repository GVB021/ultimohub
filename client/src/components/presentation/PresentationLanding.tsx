...
function normalizeLandingTextConfig(input: unknown): LandingTextConfig {
  ...
  return {
    ...
    navHubSchool: cleanSingleLine(obj.navHubSchool, defaultLandingTextConfig.navHubSchool, 16),
    navHubAlign: cleanSingleLine(obj.navHubAlign, defaultLandingTextConfig.navHubAlign, 16),
    authEnter: cleanSingleLine(obj.authEnter, defaultLandingTextConfig.authEnter, 16),
    authExit: cleanSingleLine(obj.authExit, defaultLandingTextConfig.authExit, 16),
      ...
      {canEditLandingText && isEditorOpen && (
        <div className="fixed right-4 bottom-16 z-30 w-[min(92vw,460px)] rounded-2xl border border-border bg-background/95 backdrop-blur p-4 shadow-2xl space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            <input className="rounded-md border border-border bg-background px-2 h-9 text-xs" value={textConfig.navHubDub} onChange={(e) => setTextConfig((c) => ({ ...c, navHubDub: cleanSingleLine(e.target.value, c.navHubDub, 16) }))} />
            <input className="rounded-md border border-border bg-background px-2 h-9 text-xs" value={textConfig.navHubSchool} onChange={(e) => setTextConfig((c) => ({ ...c, navHubSchool: cleanSingleLine(e.target.value, c.navHubSchool, 16) }))} />
            <input className="rounded-md border border-border bg-background px-2 h-9 text-xs" value={textConfig.navHubAlign} onChange={(e) => setTextConfig((c) => ({ ...c, navHubAlign: cleanSingleLine(e.target.value, c.navHubAlign, 16) }))} />
          </div>
          <textarea
